...
test("Tools Hub uses tile-driven panels without rendering the generic item list", () => {
  ...
  assert.match(source, />About Backups<\/h3>/);
  assert.match(source, />Export a Backup<\/h3>/);
  assert.match(source, />Restore a Backup<\/h3>/);
  assert.match(source, />Important Before Restoring<\/h3>/);
  assert.match(source, />Good Backup Practice<\/h3>/);
  assert.match(source, /does not merge them/);
...
test("Finance Hub uses tile-driven panels without rendering finance detail content by default", () => {
  ...
  assert.match(source, /Review monthly performance, expenses, purchase records, imported platform data, and preparation issues\./);
  ...
  assert.match(source, />Purchases & Invoices</);
  assert.match(source, />Financial Data Issues</);
  assert.match(source, />eBay Import & Review</);
  assert.match(source, />Year-End & EÜR Estimate</);
  assert.doesNotMatch(source, />Export Monthly JSON</);
  ...
  assert.equal(source.match(/activeTab === "finance" && activeFinancePanel === "tax_records"/g)?.length, 1);
