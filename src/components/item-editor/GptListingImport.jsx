...
export function GptListingImport({ form, setForm, sanitizeHtmlPreview }) {
  ...
  const prepared = useMemo(
    () => validationResult?.ok ? prepareGptListingUpdate(validationResult, selectedFieldIds, form) : null,
  ...
  function reviewConfirmation() {
    const nextPrepared = prepareGptListingUpdate(validationResult, selectedFieldIds, form);
  ...
  function applyToForm() {
    ...
    setForm((currentForm) => {
      const currentPrepared = prepareGptListingUpdate(validationResult, selectedFieldIds, currentForm);
      return currentPrepared.validationErrors.length ? currentForm : applyListingPackagePatchToItem(currentForm, currentPrepared.patch);
    });
    closeImport();
    setSuccessMessage("GPT listing inputs applied and final descriptions regenerated by ResellIt. Save Item to keep changes.");
  }
          ...
          <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-3xl border border-stone-200 bg-[#fffdf8] p-4 shadow-2xl sm:p-5">
            ...
            {stage === "review" && validationResult?.ok && (
              <div className="mt-4 space-y-4">
                <div className="rounded-xl border border-orange-200 bg-orange-50 p-3 text-sm text-orange-950"><p className="font-medium">Final plain and HTML descriptions will be generated by the ResellIt template.</p><p className="mt-1 text-xs">Product Description should contain only the concise product overview—not condition, testing, included items, shipping, or private-sale wording.</p></div>
            ...
            {stage === "confirm" && prepared && (
              <div className="mt-5 space-y-4">
                <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                  <h3 className="text-lg font-semibold text-stone-950">Apply {selectedCount} structured fields from GPT package?</h3>
                  ...
                  <p className="mt-2 text-sm font-semibold text-stone-800">ResellIt will regenerate the final plain and HTML descriptions. This updates the open form only; use Save Item afterward.</p>
                </div>
                <div className="flex flex-wrap justify-end gap-2">
